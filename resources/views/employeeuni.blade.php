...
<script src="/public/assets-front/js/verifyIts_employee_category.js"></script>
...
<form id="form" action="{{action('EmployeeUniController@store')}}" method="post" enctype="multipart/form-data">
    {{csrf_field()}}
    <div class="form-group">
     <input type="hidden" name="user_id" class="form-control" placeholder="Email" value="{{auth::id()}}">

    <input type="hidden" name="name" maxlength="100" class="form-control" placeholder="Ваш логин" value="{{$user->name}}">
    </div>

    <input type="text" name="title" maxlength="100" class="form-control" placeholder="Название сайта или поста"  required autofocus>
  </div>

     <input type="hidden" name="htmlpost" value="-">
	 
	 
	 <input type="hidden" name="spelling" value="-">
	
    <textarea name="email" class="form-group"  placeholder="Описание" cols="106" rows="5" style="display: inline-block;
  background: #f1f1f1; border-radius:2%; border: 1px solid #BDBDBD;" maxlength="1500" required></textarea>

    <div class="form-group">



    <input type="text" name="url" id="url" class="form-control" onclick="document.getElementById('url').value = ''" value="Нет адреса"  placeholder="Адрес сайта (если вы пишете пост, то адрес сайта не нужен)">
  ...
  <div class="form-group">
    ...
    <input type="text" name="post" id="tags" class="form-control"  placeholder="Категория темы - начните вводить и выплывит подсказка" onclick="document.getElementById('tags').value = ''" value="Нет категории" onchange="verifyIts_employee_category(this)">
  </div>
    @endif
    	@if($user -> post=='Нет категории')
