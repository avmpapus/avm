...
<script src="/assets-front/js/autocomplete_employee.js"></script>
<script src="/assets-front/js/verifyIts_employee_category.js"></script>
...
<div class="menu">
  ...
  <header>
  <img src="/images/line2.png" width="100%" height="40px" class="header">
   <div class="header">
   @if (Auth::id())
...
$("#sub_toggle").click(function() {
 ...
 @foreach ($em as $empl)
@endforeach
<form id="form" action="{{action('EmployeeUpdateController@update',$empl->id)}}" method="post" enctype="multipart/form-data">
    {{csrf_field()}}
    ...
    <input type="text" name="title" class="form-control" placeholder="Название сайта или поста" value="{{$empl->title}}" required autofocus>
  </div>


    <textarea name="email" class="form-group"  placeholder="Описание" cols="106" rows="5" style="display: inline-block;
  background: #f1f1f1; border-radius:2%; border: 1px solid #BDBDBD;" required>{{$empl->email}}</textarea>

    <div class="form-group">



    <input type="text" name="url" id="url" class="form-control" onclick="document.getElementById('url').value = ''" value="{{$empl->url}}"  placeholder="Адрес сайта (если вы пишете пост, то адрес сайта не нужен)">
  ...
  <div class="form-group">
    ...
    <input type="text" name="post" id="tags" class="form-control"  placeholder="Категория темы - начните вводить и выплывит подсказка" onclick="document.getElementById('tags').value = ''" value="{{$empl->post}}">
  </div>
			...
			<div>
				<img id="img-preview" src="/assets-front/img/upload_photo.png" />
				<br />
