...
/*
           ...
           <script src="{{asset('assets-front/js/ckeditor/ckeditor.js')}}"></script>
                   <script>
			...
			--}}
                ...
                if ($mysqli->connect_errno) {
                   ...
                   exit();
                                    ...
                                    DB::table('visit_profile_user')->insert(array('user' => auth::user()->name,
									'ip' => $_SERVER['REMOTE_ADDR'],
									'browser' => $_SERVER['HTTP_USER_AGENT'], 'day'=>date('Y-m-d H:i:s')));
...
@endif
            ...
            <small >
            <font color="#ADACAC">{{\Carbon\Carbon::parse($one_post->created_at)->format("j/m/Y в G:i")}}</font>
            ...
            {{--
            <form method="get" id="edit_post-{{$one_post->id}}">
                <div class="form-group">
                    <img src="{{ $one_post->img }}" class="img-responsive img-post-sk">
                </div>
                <div class="form-group">
                    <input type="text" class="form-control" name="title" value="{{ $one_post->title }}">
                </div>
                <div class="form-group">
                    <textarea class="form-control" name="text">{!! $one_post->description !!}</textarea>
                </div>
                <div class="form-group text-right">
                    <button style="cursor: pointer;" class="btn btn-primary btn-sm" name="save_post{{$one_post->id}}">
                        <i class="fa fa-external-link-square" aria-hidden="true"></i>
                        Сохранить
                    </button>
                    &nbsp;&nbsp;
                    <div style="cursor: pointer;" class="btn btn-danger btn-sm" id="cancel_post-{{$one_post->id}}" role="button">
                        Отменить
                    </div>
                </div>
            </form>
            ...
            <script type="text/javascript">
                $(document).ready(function() {
                    $('#edit_post-{{$one_post->id}}').hide();
                    $('#post-{{$one_post->id}}').click(function(){
                        $('#PostContent-{{$one_post->id}}').hide();
                        $('#edit_post-{{$one_post->id}}').show();
                    });
                    $('#cancel_post-{{$one_post->id}}').click(function(){
                        $('#PostContent-{{$one_post->id}}').show();
                        $('#edit_post-{{$one_post->id}}').hide();
                    });
                });
            </script>
                ...
                <table class="table-like">
                    <tr>
                        ...
                        ?>
                            ...
                            ?>
                            <td class="td-one-sk">
                                <input type='image' src='{{asset('assets-front/img/like.png')}}' value='Не нравится' data-toggle="tooltip" title="Подобається"
                                       name='submit_likes' width='20' class="radius_unlikes"
                                       data-id = "{{-- $one_post->id --}}" data-like="2"
                                       onclick="return false;" onFocus="this.blur();"/>
                            ...
                            ?>
                            <td class="td-one-sk">
                                <input type='image' src='{{asset('assets-front/img/unlike.png')}}' value='Не нравится' data-toggle="tooltip" title="Не подобається"
                                       name='submit_unlikes' width='20' class="radius_unlikes"
                                       data-id = "{{-- $one_post->id --}}" data-like="3"
                                       onclick="return false;" onFocus="this.blur();"/>
