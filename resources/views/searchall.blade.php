...
@endif
   ...
   @if(!$mobile_browser > 0)
@if (Auth::id())
   <div style="position:relative; left:-610px; top:-50px;">
<form class="example" name="search" action="{{action('SearchController@searchall')}}">
{{ csrf_field() }}
  &nbsp;&nbsp;&nbsp;<input type="text" onkeyup="this.value=this.value.replace(/^\s/,''); countChar(this)" class='search_box zbz-input-clearable zbz-input-clearable--x' placeholder="Что ищете?" name="searchString" id="tags" onchange="verifySynonyms(this)" value="<?php echo $searchString=htmlspecialchars($_GET['searchString']);?>" style="width:170%;">
...
<script>
      function countChar(val) {
        ...
        if (len >= 500) {
		  ...
		  document.getElementById("charNum1").innerHTML = "&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<font color='red'>Запроса такой длины достаточно для точного поиска!</font>";
		  }
		  if (val.value.length < 20) {
...
echo "&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<i>Если результат не оправдал ваши ожидания, предлагаем вам<br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<a href='http://go3.com.ua/public/search?_token=h7WrBlDMfVkzV7Trzl4rLYLowUl7PncxvxVCvTtG&searchString=$line_text'>$line_text</a></i>";
	 }
...
function sayHello()
{
 ...
 document.write("<h2>Здесь порно запрещено!</h2>");
 else document.write("<br>&nbsp;&nbsp;&nbsp;&nbsp;Найдите себе пару :)<br>");
}
// -->
</script>
</head>
